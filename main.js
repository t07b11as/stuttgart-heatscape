...
const observer=new IntersectionObserver(entries=>{entries.forEach(entry=>{if(!entry.isIntersecting)return;const step=Number(entry.target.dataset.step);document.body.classList.add('is-transitioning');
document.body.classList.forEach(cls=>{
  if(/^step-\d+$/.test(cls)) document.body.classList.remove(cls);
});
requestAnimationFrame(()=>{
  document.body.classList.add(`step-${step}`);
});bigNumber.textContent=step===0?'11/11':'';progressFill.style.width=`${((step+1)/steps.length)*100}%`;setBasemap(step);const view=views[step]||views[0];storyMap.flyTo(view.center,view.zoom,{duration:1.65,easeLinearity:.22});styleForStep(step)})},{threshold:.58});
