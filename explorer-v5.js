...
function comparisonSummary(a, b) {
  ...
  return `Zelle ${warmer} weist die höhere Oberflächentemperatur auf (${Math.abs(lstDiff).toFixed(1)} °C Unterschied). Zelle ${greener} besitzt den höheren NDVI-Wert und damit die stärkere Vegetationsausprägung.`;
}
  ...
  if (!comparisonB) {
    return `
      <div class="compare-card">
        <h3>Vergleich</h3>
        <p>Zelle A: <strong>${comparisonA.cell_id}</strong></p>
...
periodSelect.addEventListener('change', event => {
  ...
  updateControls();
  if (lastActiveProps) renderInfo(lastActiveProps);
  else infoCard.innerHTML = '<p class="muted">Fahre über eine Rasterzelle.</p>';
});
